...
module.exports = {
    getProjectActivity: async (data, start_date, end_date, slug) => {
                ...
                if(end_date >= created && start_date <= created) {
                    if(timeline[index].data) {
                        ...
                        users[user.id].daily_activity[day_index].activity_details.push({
                            ...
                            changes: changes,
                            timestamp: timeline[index].created,
                            link: link
                        });
        ...
        var inactivity_streaks = {}, streak_in_progress, streak_count;
        for(var id in users) {
            inactivity_streaks[id] = [];
            streak_in_progress = false;
            streak_count = 0;

            for(var day_index = 0; day_index < users[id].daily_activity.length; day_index++) {
                if(!streak_in_progress) {
                    if(users[id].daily_activity[day_index].activity_details == 0) {
                    ...
                    if(users[id].daily_activity[day_index].activity_details == 0) {
                        ...
                        if(streak_count > 1) {
                            inactivity_streaks[id].push(streak_count);
                        }
                        streak_in_progress = false;
                    }
                }
